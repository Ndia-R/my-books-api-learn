...
public interface BookChapterPageContentRepository extends JpaRepository<BookChapterPageContent, Long> {
    ...
    @Query("""
        SELECT NEW com.example.my_books_api.dto.book_chapter.BookChapterResponse(
            p.chapterNumber,
            c.title,
            COUNT(p.pageNumber)
        )
    ...
    @Query("""
        SELECT NEW com.example.my_books_api.dto.book_chapter_page_content.BookChapterPageContentResponse(
            p.bookId,
            p.chapterNumber,
            c.title,
            p.pageNumber,
